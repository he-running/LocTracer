...
import com.siho.loctracer.clusterutil.projection.Point;
import com.siho.loctracer.clusterutil.projection.SphericalMercatorProjection;
import com.siho.loctracer.clusterutil.ui.IconGenerator;
import com.siho.loctracer.clusterutil.ui.SquareTextView;
...
import static com.siho.loctracer.clusterutil.clustering.algo.NonHierarchicalDistanceBasedAlgorithm.MAX_DISTANCE_AT_ZOOM;
...
public class DefaultClusterRenderer<T extends ClusterItem> implements
     ...
     */
    private static final int MIN_CLUSTER_SIZE = 1;//原来是4个点才允许聚合
    ...
    public DefaultClusterRenderer(Context context, BaiduMap map, ClusterManager<T> clusterManager) {
        ...
        mIconGenerator.setBackground(makeClusterBackground());//-----------------可以自定义聚合的样式效果
            ...
            if (!shouldRenderAsCluster(cluster)) {
                for (T item : cluster.getItems()) {
                    ...
                    if (marker == null) {
                        ...
                        //添加锚点设置
                        markerOptions.anchor(0.51f,0.65f);
                        if (animateFrom != null) {
...
//                    .position(animateFrom == null ? cluster.getPosition() : animateFrom);
            //添加锚点设置
            MarkerOptions markerOptions = new MarkerOptions()
                    .position(animateFrom == null ? cluster.getPosition() : animateFrom).anchor(0.51f,0.65f);
